...
const WBOpCbdcStates = () => {
  ...
  useEffect(() => {
    ...
    fetch(urll)
	  ...
	  */
    issuanceservice.gettradertransactions().then((json) => {
      console.log("json", json);
      //setData(getCustomers(json.subscribertrans.centralsubcribertrans));
      console.log("json", json);
      //         const forsorting = _.uniqBy(json.tradesettlertrans, 'transactionid'); //json.transactions;
      var last = _.sortBy(json.tradesettlertrans, "updatedat").reverse();
      setData(last);
      setLoading(false);
